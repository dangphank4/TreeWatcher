...
const double DEFAULT_BAR_HEIGHT = 64;
                        ...
                        if (true) ...[
                          Expanded(
                            child: InkWell(
                              ...
                              child: Container(
                                padding:
                                // i == middleIndex? EdgeInsets.only(top: 0):
                                EdgeInsets.only(top: 12),
                                child: _buildItemWidget(
                                  items[i],
                                  i == widget.currentIndex,
                                  // showText: i != middleIndex,
                                  iconSize: i == middleIndex ? 64 : null,
                                ),
                              ),
                            ),
                          ),
  ...
  Widget _buildIcon(
    ...
    if (item.iconPath != null) {
      if (item.iconPath!.endsWith('.png')) {
        if (item.activeIconPath == item.iconPath) {
          return Image.asset(
            item.iconPath!,
            fit: BoxFit.contain,
            width: width,
